...
class BaseController extends REST_Controller {
		...
		if (is_array($data)) {
			...
			if (!array_key_exists('_id', $data)) {
				...
				if ($dataModified != null) {
					$output = ['status' => 2, 'message' => 'id already exist : ' . $data['_id'], 'data' => $data];
					return $output;
				}
			}
			foreach ($data as $key => $value) {
				if (is_array($value) && array_key_exists('_id', $value)) {
					// $key = coman_1 , objMap = [coman_,xxxx,....]
					$otherKey = '';
					if( ($x_pos = strpos($key, '_')) !== FALSE ) {
						$otherKey = substr($key, 0, $x_pos + 1);
					}
					if (array_key_exists($key, $this->objMap)) {
						$data[$key] = array('$ref' => $this->objMap[$key], '$id' => $value['_id']);
					} else if (array_key_exists($otherKey, $this->objMap)) {
						$data[$key] = array('$ref' => $this->objMap[$otherKey], '$id' => $value['_id']);
					} else {
						$data[$key] = $value['_id'];
					}
				}
	 ...
	 * @return multitype:unknown
	 */
	function saveOrUpdateMulti_post() {
		// Continue Real Operation
		$dataList = $this->_post_args;
		...
		$output = ['status' => 1, 'message' => 'successfuly executed', 'data' => $dataList];
		return $output;
	}
